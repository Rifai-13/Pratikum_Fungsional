...
def format_destination(destination):
    return f"{destination[0]} to  {destination[1]} - ${destination[2]}"

def list_destinations(destinations):
    return list(map(format_destination, destinations))

def filter_bookings(nik, bookings):
    booking_list = bookings.get(nik, [])
    return list(map(format_destination, booking_list))

def count_bookings(count):
    return count + 1

def total_bookings(nik, bookings):
    return reduce(count_bookings, bookings.get(nik, []), 0)

def rekursive_count_bookings(bookings,nik,index = 0):
    if nik not in bookings or index >= len(bookings[nik]):
        return 0
    return 1 + rekursive_count_bookings(bookings, nik, index + 1)
            ...
            if success:
                while True:
                    ...
                    if user_choice == '1':
                        ...
                        format_destination = list_destinations(DESTINATIONS)
                        for index, destination in enumerate(format_destination, start=1):
                        ...
                        filter_bookings = filter_bookings(nik, bookings)
                        print("---view Bookings Histroy----")
                        print("\n".join(filter_bookings))
                        
                        rekursive_count_bookings = rekursive_count_bookings(bookings, nik)
                        print(f"total bookings {nik} : {bookings}")
                        ...
                        total_bookings = total_bookings(nik, bookings)
